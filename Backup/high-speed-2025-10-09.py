...
"""
    ...
    def assign_stages(self):
        ...
        for index, blade_row in enumerate(self.blade_rows):
            ...
            if blade_row.type == "IGV" or blade_row.type == "OGV":
    ...
    def solve_flow(self, blade_row):
        ...
        inlet_non_dim_m_dot_rel = (
            self.m_dot * np.sqrt(c_p * T_0_rel) * blade_row.inlet.axial_velocity / blade_row.inlet.velocity
            / (blade_row.area * blade_row.inlet.p_0_rel)
        )

        print(f"{blade_row.row_type}")
        print(f"inlet_non_dim_m_dot_rel: {inlet_non_dim_m_dot_rel}")
        print(f"non_dim_m_dot_rel: {non_dim_m_dot_rel}\n")
...
def main():
    ...
    # construct engine class for the appropriate number of stages with IGV and OGV
    engine = Engine()
    r_t_in = Defaults.inlet_tip_radius
    r_t_out = Defaults.exit_tip_radius
    inlet_guide_vane = Blade_row(
        0,
        Defaults.inlet_blade_angle,
        r_t_in,
        Defaults.hub_tip_ratio,
        Defaults.stagnation_pressure_loss_coefficient
    )
    inlet_guide_vane.row_type = "Inlet Guide Vane"
    inlet_guide_vane.type = "IGV"
    engine.blade_rows.extend([inlet_guide_vane])
    ...
    outlet_guide_vane = Blade_row(
        0,
        Defaults.outlet_blade_angle,
        r_t_in,
        Defaults.hub_tip_ratio,
        Defaults.stagnation_pressure_loss_coefficient
    )
    outlet_guide_vane.row_type = "Outlet Guide Vane"
    outlet_guide_vane.type = "OGV"
    engine.blade_rows.extend([outlet_guide_vane])
